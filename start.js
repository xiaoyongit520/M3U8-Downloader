...
const _app = new Vue({
    ...
    methods: {
        startWebServer() {
            ...
            expressApp.post('/download-avgle', (req, res) => {
                let _headers = 'Referer: https://avgle.com/';
                ipcRenderer.send('task-add', {
                    url: `file:///${path.join(this.config_save_dir, req.body.fileName)}`,
                    headers: _headers,
                    myKeyIV: '',
                    taskName: req.body.fileName.replace('.m3u8', ''),
                    taskIsDelTs: true,
                    url_prefix: '',
                    generateFile: true,
                    fileData: req.body['m3u8']
                })
